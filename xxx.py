    ...
    def get_wavs(self, wav_folder):
        for root, dirs, files in os.walk(wav_folder, topdown=False):
            for file in files:
                ...
                d.load(os.path.join(root, file))
                self.phones[os.path.splitext(file)[0]] = d
        #punctuation silence
        silence_250 = simpleaudio.Audio()
        silence_250.add_echo(1, 250)
        silence_500 = simpleaudio.Audio()
        silence_500.add_echo(1, 500)
        self.phones[','] = silence_250
        self.phones['.'] = silence_500
        self.phones['?'] = silence_500
        self.phones['!'] = silence_500
...
def replace_numbers(phrase):
    ...
    for word in words:
        if word.replace('.', '', 1).isdigit():
            if '.' in word:
                phrase = phrase.replace(word, translate_float(word))
            else:
...
def replace_date(phrase):
    words = re.split("\\s+", phrase)
    for word in words:
        if re.match(r'(\d{1,2}/\d{2})', word) is not None or \
           re.match(r'(\d{1,2}/\d{2}/\d{2})', word) is not None or \
           re.match(r'(\d{1,2}/\d{2}/\d(4))', word) is not None:
            tokens = word.split('/')
            date_str = ''
            if len(tokens) == 2:
                day = int(tokens[0])
...
def get_phone_seq(phrase):
    ...
    match_phrase = replace(punctuation, "", phrase)
    match_phrase = match_phrase.lower()
    match_phrase = re.sub(r'[,]', ' ,', match_phrase)
    match_phrase = re.sub(r'[!]', ' !', match_phrase)
    match_phrase = re.sub(r'[.]', ' .', match_phrase)
    match_phrase = re.sub(r'[?]', ' ?', match_phrase)
    words = re.split("\\s+", match_phrase)
...
"""
def translate_float(d_number):
    number = d_number[:d_number.index('.')]
    str = translate_number(number)
    str += ' point'
    for s in d_number[d_number.index('.') + 1:]:
