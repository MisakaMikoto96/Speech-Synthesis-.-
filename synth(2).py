...
class Synth(object):
    ...
    def get_wavs(self, wav_folder):
        ...
        a_250 = simpleaudio.Audio()
        a_250.add_echo(1, 250)
        a_500 = simpleaudio.Audio()
        a_500.add_echo(1, 500)
        self.phones[','] = a_250
        self.phones['.'] = a_500
        self.phones['?'] = a_500
        self.phones['!'] = a_500
...
def replace_date(phrase):
    words = re.split("\\s+", phrase)
    for word in words:
        if re.match(r'(\d{1,2}/\d{2})', word) is not None or re.match(r'(\d{1,2}/\d{2}/\d{2})', word) is not None or re.match(
                r'(\d{1,2}/\d{2}/\d(4))', word) is not None:
            tokens = word.split('/')
            date_str = ''
            if len(tokens) == 2:
                day = int(tokens[0])
...
def get_phone_seq(phrase):
    ...
    punctuation = """!"#$%&'()*+-./:;<=>@[\]^_`|~"""
    m_phrase = replace(punctuation, "", phrase)
    m_phrase = m_phrase.lower()
    words = re.split("\\s+", m_phrase)
    ...
    for word in words:
        ...
        if word == ',' or word == '.' or word == '?' or word == '!':
            ...
            if tmp in cmudict.dict():
                ...
                print("error: ",tmp," not in the dict.")
